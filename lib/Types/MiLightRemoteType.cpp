...
static const char* REMOTE_NAME_RGB_SAT = "rgb_sat";
...
const MiLightRemoteType MiLightRemoteTypeHelpers::remoteTypeFromString(const String& type) {
  ...
  if (type.equalsIgnoreCase(REMOTE_NAME_RGB_SAT)) {
    return REMOTE_TYPE_RGB_SAT;
  }
...
const String MiLightRemoteTypeHelpers::remoteTypeToString(const MiLightRemoteType type) {
  switch (type) {
    ...
    case REMOTE_TYPE_RGB_SAT:
      return REMOTE_NAME_RGB_SAT;
    default:
